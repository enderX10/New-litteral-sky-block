...
public class LSBClient {
    ...
    public static void drawUI(GuiGraphics graphics) {
        ...
        // Poprawiony potok renderowania dla 1.20.6
        BufferBuilder builder = Tesselator.getInstance().getBuilder();
        builder.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_TEX);
        builder.vertex(0F, 0F, 10F).uv(0F, 1F).endVertex();
        builder.vertex(0F, h, 10F).uv(0F, 0F).endVertex();
        builder.vertex(w, h, 10F).uv(1F, 0F).endVertex();
        builder.vertex(w, 0F, 10F).uv(1F, 1F).endVertex();
